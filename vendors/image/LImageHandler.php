...
/**
 * Image handler
 * @author Yaroslav Pelesh aka Tokolist http://tokolist.com. Extended by Victor Zinchenko aka lukesky. Extended by Prudnikov Sergey aka grinSpike
...
class LImageHandler
{
    ...
    public function textWithOutline($text, $fontFile, $size = 12, $color = array(0, 0, 0),$colorOutline = array(0, 0, 0), $corner = self::CORNER_LEFT_TOP, $offsetX = 0, $offsetY = 0, $angle = 0, $alpha = 0)
    {
        $this->checkLoaded();
        ...
        // обводка текста
        $color_outline = array(1, 1, 1);
        if ($alpha > 0) {
            $colorOutline = imagecolorallocatealpha($this->image, $colorOutline[0], $colorOutline[1], $colorOutline[2], $alpha);
        } else {
            $colorOutline = imagecolorallocate($this->image, $colorOutline[0], $colorOutline[1], $colorOutline[2]);
        }
        $ow = 3;//outline width
        $od = 2;//outline diagonal
        
        imagettftext($this->image, $size, $angle, $posX+$ow, $posY + $textHeight, $colorOutline, $fontFile, $text);
        imagettftext($this->image, $size, $angle, $posX-$ow, $posY + $textHeight, $colorOutline, $fontFile, $text);
        imagettftext($this->image, $size, $angle, $posX, $posY + $textHeight+$ow, $colorOutline, $fontFile, $text);
        imagettftext($this->image, $size, $angle, $posX, $posY + $textHeight-$ow, $colorOutline, $fontFile, $text);
        imagettftext($this->image, $size, $angle, $posX+$od, $posY + $textHeight+$od, $colorOutline, $fontFile, $text);
        imagettftext($this->image, $size, $angle, $posX+$od, $posY + $textHeight-$od, $colorOutline, $fontFile, $text);
        imagettftext($this->image, $size, $angle, $posX-$od, $posY + $textHeight-$od, $colorOutline, $fontFile, $text);
        imagettftext($this->image, $size, $angle, $posX-$od, $posY + $textHeight+$od, $colorOutline, $fontFile, $text);
